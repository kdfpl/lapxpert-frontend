...
<template>
  <div class="relative w-full mt-5">
    ...
    <div class="w-full h-50 overflow-hidden rounded-lg">
      <img class="w-full h-full object-cover" :src="image" alt="Banner" />
    </div>
    ...
    <div class="flex items-center max-w-md shadow-md bg-white/60 rounded-md">
      <div class="w-60">
        <input v-model="search" type="search"
          class="w-full px-5 py-1 border-0 bg-white text-gray-900 focus:outline-none" placeholder="search" />
      </div>
      <div>
        <button type="submit" class="flex items-center justify-center w-12 h-12 text-white rounded-r-lg"
          :class="searchLength > 0 ? 'bg-black' : 'bg-gray-500 cursor-not-allowed'" :disabled="searchLength === 0">
  ...
  <div class="flex justify-center sticky top-[100vh] mt-5 items-center">
    <customTable :headers="headers" :data="data" :rowHeight="rowHeight" :detailLink="'/chi-tiet-hoa-don'" />
  </div>
</template>
