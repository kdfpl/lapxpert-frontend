...
onMounted(async () => {
  if (route.params.id) { // Chỉ khi có id mới thực hiện sửa
    ...
    const mauSacList = await getAllMauSac();
    const selectedMauSac = mauSacList.find((msac) => msac.id == route.params.id);
    if (selectedMauSac) {
      msacData.value = { ...selectedMauSac };
    }
  }
});
...
const handleSubmit = async () => {
  try {
    if (isEditing.value) {
      await updateMauSac(msacData.value.id, msacData.value);
      alert("Cập nhật Color thành công!");
    } else {
      await addMauSac(msacData.value);
      alert("Thêm Color thành công!");
    }
    router.push("/admin/mau-sac"); // Điều hướng về danh sách
  } catch (error) {
    console.error("Lỗi khi thêm/sửa Color:", error);
...
<template>
  <div class="relative w-full mt-5">
    ...
    <div
      ...
      <h1 class="text-3xl font-bold">THÊM/SỬA MÀU SẮC</h1>
      <button
        ...
        <router-link to="/admin/mau-sac">Danh Sách</router-link>
      </button>
    </div>
  </div>
  <div class="flex">
    <div class="flex-1 mr-5 w-140 bg-gray-50 p-4 rounded-lg bg-white border rounded">
      <h3 class="text-lg font-semibold mb-4">{{ isEditing ? "Sửa Color" : "Thêm Color" }}</h3>
      <input v-model="msacData.maMau" placeholder="Mã Màu" class="border p-2 w-full mb-2" required />
      <input v-model="msacData.tenMau" placeholder="Tên Màu" class="border p-2 w-full mb-2" required />
