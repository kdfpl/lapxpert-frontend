<template>
  ...
  <div :class="[
    ...
    <div class="flex justify-between items-center gap-4 p-4">
      ...
      <button @click="toggleSidebar" class="rounded  hover:bg-gray-300 transition">
    ...
    <ul class="flex-1 mt-10">
      <li v-for="(menu, index) in menuItems" :key="index">
        <div class="group">
          ...
          <button v-if="menu.subItems" @click="toggleSubMenu(index)"
            ...
            <FontAwesomeIcon v-if="!isCollapsed" :icon="showSubItems[index] ? faChevronDown : faChevronRight"
              class="size-4 transition-transform duration-200" :class="{ 'rotate-90': showSubItems[index] }" />
          </button>

          <!-- Nếu không có submenu, dùng RouterLink -->
          <RouterLink v-else :to="menu.link" class="flex items-center p-3 w-full hover:bg-gray-300 rounded transition">
            <FontAwesomeIcon :icon="menu.icon" class="text-2xl" />
            <span v-if="!isCollapsed" class="ml-3">{{ menu.name }}</span>
          </RouterLink>

          <!-- Submenu -->
          <ul v-if="menu.subItems && showSubItems[index] && !isCollapsed" class="pl-6">
            <li v-for="(subMenu, subIndex) in menu.subItems" :key="subIndex">
              <div class="group">
                <RouterLink v-if="!subMenu.subItems" :to="subMenu.link"
                ...
                <button v-else @click="toggleSubMenu(subMenu.name)"
                  class="flex items-center justify-between p-2 w-full hover:bg-gray-200/50 rounded transition">
                  <div class="flex items-center">
                    <FontAwesomeIcon :icon="subMenu.icon" class="" />
                    <span class="ml-3">{{ subMenu.name }}</span>
                  </div>
                  <FontAwesomeIcon :icon="showSubItems[subMenu.name] ? faChevronDown : faChevronRight"
                    class=" transition-transform duration-200" :class="{ 'rotate-90': showSubItems[subMenu.name] }" />
                </button>

                <!-- Sub-submenu -->
                <ul v-if="subMenu.subItems && showSubItems[subMenu.name]" class="pl-6">
                  <li v-for="(subSubMenu, subSubIndex) in subMenu.subItems" :key="subSubIndex">
                    <RouterLink :to="subSubMenu.link"
                      class="flex items-center p-2 hover:bg-gray-200/50 rounded transition">
