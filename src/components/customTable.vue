      ...
      <table class="w-full border-collapse">
        ...
        <tbody>
          <tr v-for="(row, rowIndex) in paginatedData" :key="rowIndex"
            ...
            <td v-for="(cell, key) in row" :key="key" class="px-4 py-3 text-black border-b border-[#C8C7C7]" :class="rowHeight">
              <slot :name="`cell-${key}`" :item="row">
                <!-- Nếu không có slot, hiển thị dữ liệu mặc định -->
                {{ cell }}
              </slot>
            </td>
          </tr>
