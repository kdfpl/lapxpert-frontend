      ...
      <table class="w-full border-collapse">
        ...
        <tbody>
          <tr v-for="(row, rowIndex) in paginatedData" :key="rowIndex"
            ...
            <td v-for="(cell, cellIndex) in row" :key="cellIndex" class="px-4 py-3 text-black border-b border-[#C8C7C7]"
              :class="rowHeight">
            ...
            <td class="px-4 py-3 text-black border-b border-[#C8C7C7]">
              <router-link :to="`${detailLink}/${row.id}`">
                <button
              ...
              <button @click="handleDelete(paginatedData[rowIndex])"
    ...
    <div class="mt-4 flex justify-between items-center">
      ...
      <div class="flex space-x-2">
        ...
        <button v-for="page in totalPages" :key="page" @click="setPage(page)"
          class="px-3 py-1 rounded-lg border shadow-md transition duration-300" :class="currentPage === page
...
const handleDelete = (row) => {
  if (confirm("Bạn có chắc muốn xóa không?")) {
    if (typeof props.deleteFunc
      === "function"
    ) {
      props.deleteFunc(row)
    } else {
      console.error(props.deleteFunc)
    }
  }
};
